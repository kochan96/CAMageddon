...
namespace CAMageddon {
	...
	void OpenGLShader::CheckIfUniformExists(const std::string& name, int uniformLocation)
	{
		if (uniformLocation == -1)
			LOG_WARNING("Uniform {} in shader {} not found", name.c_str(), m_Name.c_str());
	}

	void OpenGLShader::UploadUniformInt(const std::string& name, int value)
	{
		GLint location = glGetUniformLocation(m_RendererID, name.c_str());
		CheckIfUniformExists(name, location);
